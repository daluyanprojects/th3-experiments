...
# Training depth normalization bounds
DEPTH_MIN_TRAINING = 5.0
DEPTH_MAX_TRAINING = 78.0
...
def build_conditioning_vector(
    ...
    """
    Build 19-dimensional conditioning vector for flood prediction model.
    
    Args:
        storm_type: one of ['front-loaded', 'balanced', 'back-loaded', 'triangular']
        depth_mm: total storm depth in mm
        rain_min: minimum rainfall intensity (for normalization)
        rain_max: maximum rainfall intensity (for normalization)
        tpeak: peak time fraction (0.1-0.9) for triangular storms only
    
    Returns:
        (conditioning_vector, warnings) where:
        - conditioning_vector: (19,) numpy array
          [0:13] normalized rainfall intensities
          [13:17] storm type one-hot encoding
          [17] r parameter (tpeak for triangular, 0.0 for SCS)
          [18] normalized total depth
        - warnings: list of validation warnings
    
    Example:
        >>> cond_vec, warnings = build_conditioning_vector(
    ...
    # 5. r parameter (0.0 for SCS, tpeak for triangular)
    r_param = np.float32(tpeak if storm_type == 'triangular' else 0.0)
    
    # 6. Normalize depth to [0, 1] using training bounds
    depth_norm = (depth_mm - DEPTH_MIN_TRAINING) / (DEPTH_MAX_TRAINING - DEPTH_MIN_TRAINING + 1e-8)
    depth_norm = np.float32(depth_norm)

    # 7. Concatenate → (19,)
    conditioning = np.concatenate([
        norm_sequence,           # (13,) rainfall
        onehot,                  # (4,) storm type
        [r_param],               # (1,) r parameter
        [depth_norm],            # (1,) normalized depth
    ])
