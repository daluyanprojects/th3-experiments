...
class RainfallSequenceEmbedding(nn.Module):
    def __init__(self, num_timesteps: int, embed_dim: int, hidden_dim: int, method: str):
        ...
        if method == 'conv':
            # 1D Convolution to process temporal sequence
            self.temporal_encoder = nn.Sequential(
                nn.Conv1d(1, hidden_dim, kernel_size=3, padding=1),
                nn.GELU(),
                nn.Conv1d(hidden_dim, hidden_dim, kernel_size=3, padding=1),
                nn.GELU(),
                nn.AdaptiveAvgPool1d(1),  # Pool to single value
                nn.Flatten()
            )
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.method == 'conv':
            # Reshape for 1D conv: (batch, 13) -> (batch, 1, 13)
            x = x.unsqueeze(1)
            # Encode: (batch, 1, 13) -> (batch, hidden_dim)
            x = self.temporal_encoder(x)
            # Project: (batch, hidden_dim) -> (batch, embed_dim)
            x = self.projection(x)
            # Add sequence dimension: (batch, embed_dim) -> (batch, 1, embed_dim)
            x = x.unsqueeze(1)
            
        elif self.method == 'mlp':
            # Direct MLP: (batch, 13) -> (batch, embed_dim)
            x = self.mlp(x)
            # Add sequence dimension: (batch, embed_dim) -> (batch, 1, embed_dim)
            x = x.unsqueeze(1)
            
        elif self.method == 'attention':
            # Embed each timestep: (batch, 13) -> (batch, 13, 1) -> (batch, 13, hidden_dim)
            x = x.unsqueeze(-1)
            x = self.timestep_embed(x)
            # Self-attention over timesteps
            x, _ = self.temporal_attention(x, x, x)
            # Pool over timesteps: (batch, 13, hidden_dim) -> (batch, hidden_dim)
            x = x.mean(dim=1)
            # Project: (batch, hidden_dim) -> (batch, embed_dim)
            x = self.projection(x)
            # Add sequence dimension: (batch, embed_dim) -> (batch, 1, embed_dim)
...
class ViTFloodClassifier(nn.Module):
    def __init__(
        ...
        # Positional encoding (2 tokens: rainfall + patch)
        num_positions = 2
