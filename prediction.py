...
# ── Helpers (unchanged from ViT prediction.py) ─────────────────────────────────
...
def reconstruct_ground_truth_maps_cnn(
    test_dataset   : FloodMapDataset,
    y_test         : np.ndarray,        # (S, H, W)
    scenario_ids   : Optional[List[int]] = None,
) -> Dict[int, np.ndarray]:
    """
    CNN version: y_test is already (S, H, W) — no patch reassembly needed.
    Outside-mask pixels are -1 (set during reconstruct_for_cnn).

    Parameters
    ----------
    test_dataset : FloodMapDataset  — used to get S and map shape
    y_test       : (S, H, W) int   — label maps, -1 = outside mask
    scenario_ids : list of ints    — scenario IDs matching y_test order;
                                     defaults to [1, 2, ..., S]

    Returns
    -------
    gt_maps : {scenario_id: (H, W) int8 array}
    """
    print("\n[Reconstructing Ground Truth Maps — CNN]")

    S, H, W = y_test.shape
    if scenario_ids is None:
        scenario_ids = list(range(1, S + 1))

    assert len(scenario_ids) == S, \
        f"scenario_ids length {len(scenario_ids)} != y_test scenarios {S}"

    gt_maps = {}
    for s_idx, sid in enumerate(scenario_ids):
        gt_maps[sid] = y_test[s_idx].astype(np.int8)   # (H, W)
        valid = (y_test[s_idx] >= 0).sum()
...
@torch.no_grad()
def generate_prediction_maps_cnn(
    ...
    """
    CNN version: model outputs (B, 5, H, W) directly.
    No patch coordinate assembly needed — argmax the output map per scenario.

    Parameters
    ----------
    model        : trained CNNFloodModel
    test_loader  : DataLoader over test_dataset (batch_size=1 recommended)
    test_dataset : FloodMapDataset — for scenario count and shape
    cfg          : CNNTrainConfig
    scenario_ids : list mapping DataLoader order → scenario IDs
    spatial_shape: expected (H, W) of each map

    Returns
    -------
    {'predictions': {scenario_id: {'prediction_map', 'probability_map', 'confidence_map'}}}
    """
    ...
    assert len(scenario_ids) == S, \
        f"scenario_ids length {len(scenario_ids)} != test_dataset size {S}"

    print("\n" + "=" * 60)
    print("PREDICTION PIPELINE (CNN)")
    print("=" * 60)
    ...
    for batch_idx, (spatial, rainfall, conditioning, labels) in enumerate(
        ...
        probs     = torch.softmax(logits, dim=1)             # (B, 5, H, W)
        preds     = logits.argmax(dim=1)                     # (B, H, W)

        # DataLoader may yield batches of size > 1 — iterate within batch
        B = spatial.size(0)
        for b in range(B):
            # Global scenario index across all batches
            s_global = batch_idx * test_loader.batch_size + b
            if s_global >= S:
                break
            sid = scenario_ids[s_global]

            pred_map = preds[b].cpu().numpy().astype(np.int8)        # (H, W)
            prob_map = probs[b].cpu().numpy().astype(np.float32)     # (5, H, W)
            conf_map = prob_map.max(axis=0)                           # (H, W)

            # Mask outside pixels back to -1 using the label map
            outside_mask          = labels[b].numpy() == cfg.ignore_index
            pred_map[outside_mask] = -1
            ...
            valid_conf = conf_map[~outside_mask]
            if len(valid_conf):
                all_conf.append(valid_conf)

            valid = (~outside_mask).sum()
            print(f"  Scenario {sid}: {valid:,} valid pixels")
...
# ── 4. DEM overlay plots (identical to ViT) ───────────────────────────────────
...
def _plot_individual(
    ...
    ax.set_title(f'Predicted Flood Map — Scenario {sid}',
...
def _plot_prediction_grid(
    ...
    plt.suptitle('Predicted Flood Maps — All Test Scenarios',
                 fontsize=13, fontweight='bold')
