...
# ── Shared helpers (unchanged from CNN prediction.py) ─────────────────────────
...
def reconstruct_ground_truth_maps_xgb(
    test_scenario_meta : List[Dict],   # list of dicts with 'scenario_id', 'patch_coord'
    y_test             : np.ndarray,   # (N_test,)  flat patch labels
    test_scenario_ids  : List[int],    # ordered list of 15 unique scenario IDs
    patch_size         : int = 4,
    ...
    XGBoost operates on flat patch arrays — unlike CNN which has (S, H, W)
    labels directly, we must rebuild the spatial map by placing each patch
    at its (i, j) coordinate from scenario_metadata.

    Parameters
    ----------
    test_scenario_meta : list of dicts, one per test patch
                         Each dict must have 'scenario_id' and 'patch_coord' (i, j).
    y_test             : (N_test,) int  — flat label array, one entry per patch
    test_scenario_ids  : list of ints   — 15 scenario IDs in the order they appear
    patch_size         : int            — 4 (matches ViT/CNN)
    spatial_shape      : (H, W)         — (1152, 1152)

    Returns
    -------
    gt_maps : {scenario_id: (H, W) int8}  — -1 = outside mask
    """
...
def generate_prediction_maps_xgb(
    y_pred_flat        : np.ndarray,   # (N_test,)    predicted class per patch
    y_proba_flat       : np.ndarray,   # (N_test, 5)  class probabilities
    ...
    """
    Reassemble flat XGBoost predictions into per-scenario (H, W) maps.

    Unlike the CNN which outputs (B, 5, H, W) tensors in a DataLoader loop,
    XGBoost predict() returns a flat (N,) array. This function places each
    patch prediction at its spatial coordinate using scenario_metadata.

    Parameters
    ----------
    y_pred_flat        : (N_test,)    int   — model.predict(X_test_feat)
    y_proba_flat       : (N_test, 5) float  — model.predict_proba(X_test_feat)
    test_scenario_meta : list of dicts with 'scenario_id', 'patch_coord'
    test_scenario_ids  : list of 15 scenario IDs
    cfg                : XGBConfig
    patch_size         : int  — 4
    spatial_shape      : (H, W) — (1152, 1152)
    ...
    {'predictions': {scenario_id: {
        'prediction_map' : (H, W) int8,     -1 = outside mask
        'probability_map': (H, W, 5) float, 0  = outside mask
        'confidence_map' : (H, W) float,    max-class probability
    }}}
    """
    ...
    H, W = spatial_shape
    print("\n" + "=" * 60)
    print("PREDICTION PIPELINE (XGBoost)")
    print("=" * 60)
    ...
    if all_conf:
        ...
        _plot_confidence_histogram(flat, output_dir)

    print(f"\n✓ Done — results in: {output_dir}")
    return {'predictions': prediction_maps}


# ── 3. Evaluation (identical logic to CNN version) ────────────────────────────
def build_eval_results(
    ...
    """
    Compute per-scenario accuracy, F1, IoU, precision, recall.
    Identical to CNN build_eval_results — operates on (H, W) numpy arrays.
    pred_maps_dict values should be dicts with key 'prediction_map', OR
    plain (H, W) arrays — both formats accepted.
    """
    ...
    for sid in scenario_ids:
        ...
        # Accept both {'prediction_map': arr} and raw arr
        pm = pred_maps_dict[sid]
        pred = pm['prediction_map'] if isinstance(pm, dict) else pm

        valid = (gt >= 0) & (pred >= 0)
        acc   = float((gt[valid] == pred[valid]).mean()) if valid.any() else 0.0
    ...
    f1s_list  = [m['f1_macro']  for m in metrics]
    best_idx  = int(np.argmax(f1s_list))
    worst_idx = int(np.argmin(f1s_list))

    print(f"\n✓ Eval: {len(scenario_ids)} scenarios  |  "
          f"Best  RS{scenario_ids[best_idx]}  F1={f1s_list[best_idx]:.3f}  |  "
          f"Worst RS{scenario_ids[worst_idx]} F1={f1s_list[worst_idx]:.3f}")
...
# ── 4. DEM overlay plots (unchanged from CNN prediction.py) ───────────────────
...
# ── Internal plot helpers (unchanged from CNN version) ────────────────────────
def _plot_individual(
    ...
    ax.set_title(f'Predicted Flood Map — Scenario {sid} (XGBoost)',
...
def _plot_prediction_grid(
    ...
    plt.suptitle('Predicted Flood Maps — All Test Scenarios (XGBoost)',
                 fontsize=13, fontweight='bold')
...
def _plot_confidence_histogram(flat: np.ndarray, output_dir: Path):
    ...
    ax.set(xlabel='Confidence', ylabel='Frequency',
           title='Prediction Confidence Distribution (XGBoost)')
